...
export default function TestimonialsSection() {
  ...
  return (
    <section className="relative py-20 lg:py-32 bg-[#0A0F0C] overflow-hidden" id="testimonials">
      {/* gold accent glow */}
      <div className="absolute top-0 left-[-100px] w-[420px] h-[420px] rounded-full bg-[#E8C468] opacity-[0.05] blur-[120px] pointer-events-none" />
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        <div className="text-center mb-16">
          <span className="caption text-gradient-gold mb-4 block">EARLY ACCESS</span>
          <h2 className="h1-text mb-4">We&apos;re onboarding our first brokers</h2>
          <p className="text-[16px] text-[#9FB0A6] max-w-[560px] mx-auto">
        ...
        <div className="max-w-3xl mx-auto">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              <div key={i} className="text-center p-4 rounded-lg glass-card">
                <p className="text-[28px] font-display font-bold text-gradient-emerald">{stat.value}</p>
                <p className="text-[12px] text-[#9FB0A6] mt-1">{stat.label}</p>
                {stat.note && <p className="text-[11px] text-[#6FE3B0] mt-0.5">{stat.note}</p>}
              </div>
            ))}
          </div>

          <p className="text-[12px] text-[#6B7C73] text-center mt-3 font-mono">
            {loaded
        ...
        <div className="relative max-w-3xl mx-auto mt-14">
          <div className="relative p-8 md:p-10 rounded-2xl glass-card overflow-hidden">
            {/* corner glow */}
            <div className="absolute -top-20 -right-20 w-64 h-64 rounded-full bg-[#34D399]/10 blur-[80px] pointer-events-none" />
            <ShieldCheck className="absolute top-6 left-6 w-8 h-8 text-[#34D399]/40" />
            ...
            <p className="text-[16px] md:text-[18px] text-[#D5E0D9] leading-relaxed mb-8">
            ...
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-full bg-gradient-to-br from-[#34D399] to-[#1B4332] flex items-center justify-center text-[#0A0F0C] text-[16px] font-bold shadow-[0_0_24px_rgba(52,211,153,0.4)]">
                LB
              </div>
              <div>
