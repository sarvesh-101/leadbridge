  ...
  return (
    <section className="relative py-20 lg:py-32 bg-[#0B0D12]" ref={ref}>
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <p className="text-[15px] text-[#8B93A3] max-w-[520px] mx-auto mb-12">
        ...
        <div className="relative inline-block">
              ...
              {cities.map((city, i) => (
                <g key={`${city.name}-${i}`}>
                  <circle
                    ...
                    fill={city.status === "taken" ? "#3B82F6" : "#363B45"}
                    style={{ cursor: "pointer", opacity: 0 }}
                    onMouseEnter={() => setHoveredCity(city)}
                    onMouseLeave={() => setHoveredCity(null)}
                  />
                  {city.status === "taken" && (
                    <circle cx={city.x} cy={city.y} r={8} fill="none" stroke="#3B82F6" strokeWidth="1" opacity="0.3" className="animate-sonar-ring" style={{ animationDelay: `${i * 0.1}s` }} />
                  )}
                </g>
              ))}
            </g>
          ...
          {hoveredCity && (
            <div className="absolute z-10 pointer-events-none inset-0 flex items-start justify-center pt-4">
              <div className="px-3 py-2 rounded border border-[#272B34] bg-[#14161C] text-[13px] whitespace-nowrap">
                <span className="text-[#F2F4F8] font-medium">{hoveredCity.name}</span>
                <span className={`ml-2 ${hoveredCity.status === "taken" ? "text-[#3B82F6]" : "text-[#10B981]"}`}>
          ...
          {!loaded && (
            <div className="absolute inset-0 flex items-center justify-center">
              <span className="text-[13px] text-[#363B45] font-mono">Loading territory data…</span>
            </div>
          )}
          {loaded && cities.length === 0 && (
            <div className="absolute inset-0 flex items-center justify-center">
              <span className="text-[13px] text-[#363B45] font-mono max-w-[260px]">
        ...
        <div className="flex flex-wrap items-center justify-center gap-4 mt-12">
          <div className="px-6 py-3 rounded-full border border-[#272B34] bg-[#14161C] text-[14px] text-[#8B93A3] font-mono">
            <span className="text-[#F2F4F8] font-semibold">{stats?.totalCitiesTracked ?? "—"}</span> cities tracked
          </div>
          <div className="px-6 py-3 rounded-full border border-[#272B34] bg-[#14161C] text-[14px] text-[#8B93A3] font-mono">
            <span className="text-[#3B82F6] font-semibold">{stats?.citiesClaimed ?? "—"}</span> claimed
          </div>
          <div className="px-6 py-3 rounded-full border border-[#272B34] bg-[#14161C] text-[14px] text-[#8B93A3] font-mono">
            <span className="text-[#10B981] font-semibold">{stats?.citiesAvailable ?? "—"}</span> available
          </div>
        </div>
      </div>
    </section>
