...
export default function AppointmentsPage() {
  ...
  // Quick stats
  const todayCount = bookings.filter((b: Booking) => {
        ...
        {loading ? (
          ...
          filtered.map((apt: Booking, i: number) => (
            ...
            >
              <div className="flex items-center gap-3 flex-1 min-w-0">
              ...
              </div>              <div className="flex items-center gap-3 sm:flex-col sm:items-end">
                ...
                {apt.lead?.score != null && <div className="text-xs text-gray-500">Score: {apt.lead.score}</div>}
              </div>
            </motion.div>
          ))
        ) : (
