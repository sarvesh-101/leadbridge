    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-xs text-gray-400 hover:bg-white/10 hover:text-white transition-all flex items-center gap-1.5"
        >
          ...
          : statCards.map((card, i) => (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <span className="text-[11px] font-medium text-gray-500">{card.title}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="p-5 rounded-xl bg-white/5 border border-white/10">
          ...
          {loading ? (
            <div className="space-y-3">{[1,2,3,4].map(i => <div key={i} className="h-6 bg-white/5 rounded animate-pulse" />)}</div>
          ) : (
            <div className="grid grid-cols-2 gap-4">
              {[
                { label: "Total Leads", value: totalLeads, color: "text-blue-400", bg: "bg-blue-500/10" },
                { label: "Pending", value: pendingCount, color: "text-gray-400", bg: "bg-gray-500/10" },
                ...
                <div key={item.label} className={cn("p-3 rounded-lg border border-white/5", item.bg)}>
        ...
        <div className="p-5 rounded-xl bg-white/5 border border-white/10">
          ...
          {loading ? (
            <div className="space-y-3">{[1,2,3,4,5].map(i => <div key={i} className="h-5 bg-white/5 rounded animate-pulse" />)}</div>
            ...
            <div className="space-y-2.5">
              {data.leadsBySource.map((s, i) => (
                <div key={s.source} className="flex items-center gap-3">
                  ...
                  <span className="text-xs text-gray-400 flex-1 capitalize">{s.source || "Unknown"}</span>
                  <span className="text-xs text-white font-medium">{s._count?.id || 0}</span>
            ...
            <p className="text-xs text-gray-500 text-center py-6">No lead data yet</p>
          )}
        </div>
      </div>
