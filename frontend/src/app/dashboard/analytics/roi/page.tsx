...
export default function ROIPage() {
  ...
  const trendIcon = (trend: string) => {
    ...
    return <Minus className="w-3.5 h-3.5 text-[#9FB0A6]" />;
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-[#F0F7F3]">Lead Source ROI</h1>
        <p className="text-[#9FB0A6] mt-1">Cost-per-lead and conversion analytics by source</p>
      </div>
      ...
      {/* Source ROI Table */}
      {loading ? (
        <div className="space-y-3">{[1,2,3].map(i => <div key={i} className="h-16 rounded-xl app-card animate-pulse" />)}</div>
      ) : sources.length === 0 ? (
        <div className="text-center py-16 rounded-xl app-card">
          <DollarSign className="w-12 h-12 mx-auto mb-4 text-[#9FB0A6]" />
          <h3 className="text-lg font-medium text-[#F0F7F3] mb-2">No source data yet</h3>
          <p className="text-sm text-[#9FB0A6]">Leads will appear here once they start coming in</p>
        </div>
      ) : (
        <div className="space-y-4">
          {/* Header */}
          <div className="hidden lg:grid grid-cols-7 gap-4 px-4 py-2 text-xs text-[#9FB0A6] font-medium">
            <span>Source</span>
          ...
          {sources.map((src: any, i: number) => (
            <motion.div key={src.source} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}
              className="grid grid-cols-2 lg:grid-cols-7 gap-3 p-4 rounded-xl app-card app-card-hover hover:bg-white/[0.06] transition-all items-center"
            >
              <div className="lg:col-span-1">
                <span className="text-sm font-medium text-[#F0F7F3] capitalize">{src.source}</span>
              </div>
              <div className="text-right">
                <div className="text-sm text-[#F0F7F3] font-medium">{src.totalLeads}</div>
                <div className="text-[10px] text-gray-600">leads</div>
              </div>
              <div className="text-right">
                <div className="text-sm text-[#F0F7F3] font-medium">{src.conversionRate}%</div>
                ...
                {editingSource === src.source ? (
                  <div className="flex items-center gap-1 justify-end">
                    <input type="number" value={costInput} onChange={(e) => setCostInput(e.target.value)}
                      className="w-20 px-2 py-1 rounded-lg app-card text-xs text-[#F0F7F3] text-right"
                      autoFocus
                    />
                    <button onClick={() => updateCost(src.source)} className="p-1 text-green-400 text-xs">OK</button>
                    <button onClick={() => setEditingSource(null)} className="p-1 text-[#9FB0A6] text-xs">X</button>
                  </div>
                ) : (
                  <div className="flex items-center justify-end gap-1">
                    <span className="text-sm text-[#F0F7F3]">₹{src.costPerLead}</span>
                    <button onClick={() => { setEditingSource(src.source); setCostInput(String(src.costPerLead)); }}
                      className="p-0.5 text-[#9FB0A6] hover:text-[#F0F7F3] opacity-0 group-hover:opacity-100 transition-all"
                    >
              ...
              <div className="text-right text-sm text-[#9FB0A6]">₹{src.costPerConversion}</div>
              <div className="text-right text-sm text-[#9FB0A6]">₹{src.totalCost?.toLocaleString()}</div>
              <div className="flex items-center justify-end">
                {trendIcon(src.trend)}
                <span className="text-xs text-[#9FB0A6] ml-1 capitalize">{src.trend}</span>
              </div>
            </motion.div>
          ))}
        </div>
