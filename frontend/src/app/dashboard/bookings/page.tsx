  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button onClick={() => {
          exportToCSV(bookings, `bookings-${new Date().toISOString().split("T")[0]}`, EXPORT_HEADERS.bookings);
          toast.success(`${bookings.length} bookings exported`);
        }} disabled={bookings.length === 0}
          className="flex items-center gap-2 px-4 py-2 rounded-lg border border-white/10 text-gray-300 text-sm font-medium hover:bg-white/10 disabled:opacity-50"
        >
      ...
      <div className="space-y-3">
        {loading ? (
            ...
            >
              <div className="flex items-center gap-3 flex-1 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3 text-xs text-gray-500 mt-1">
                    <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> {formatDate(apt.visitDate)} {apt.visitTime}</span>
                    <span className="flex items-center gap-1 truncate"><MapPin className="w-3 h-3" /> {apt.propertyAddress || apt.propertyName || "—"}</span>
                  </div>
              ...
              <div className="flex items-center gap-3 sm:flex-col sm:items-end">
                <span className={cn("inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium",
                  ...
                  apt.status === "REMINDED" && "bg-teal-500/10 text-teal-400",
                  apt.status === "VISITED" && "bg-green-500/10 text-green-400",
                  apt.status === "NO_SHOW" && "bg-red-500/10 text-red-400",
                  apt.status === "RESCHEDULED" && "bg-yellow-500/10 text-yellow-400",
          ...
          <EmptyState title={search ? "No bookings match your search" : "No bookings yet"}
            description={search ? "Try a different search term" : "Bookings will appear here when leads book visits with the AI agent"}
          />
        )}
      </div>

      <Pagination currentPage={page} totalPages={totalPages} total={total} pageSize={20} onPageChange={setPage} loading={loading} />
    </div>
  );
}
