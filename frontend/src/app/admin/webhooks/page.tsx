...
export default function AdminWebhooksPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-[#F0F7F3]">Webhook Monitor</h1>
        <p className="text-[#9FB0A6] mt-1">Platform-wide webhook health and activity</p>
      </div>
  ...
  if (loading) {
    return (
      <div className="space-y-3">
        {[1, 2, 3].map(i => <div key={i} className="h-16 rounded-xl app-card animate-pulse" />)}
      </div>
    );
  }
  ...
  if (!health) {
    return (
      <div className="text-center py-16 rounded-xl app-card">
        <ShieldAlert className="w-12 h-12 mx-auto mb-4 text-[#9FB0A6]" />
        <h3 className="text-lg font-medium text-[#F0F7F3] mb-2">No data available</h3>
        <p className="text-sm text-[#9FB0A6]">Webhook health data will appear once events start flowing</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className={cn(
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-xs text-[#9FB0A6]">Last 24 hours</p>
          </div>
          <span className={cn(
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            <div key={s.label}>
              ...
              <p className="text-xs text-[#9FB0A6]">{s.label}</p>
            </div>
          ))}
        </div>
        ...
        {health.byEndpoint && Object.keys(health.byEndpoint).length > 0 && (
          <div className="mt-5 pt-4 border-t border-white/10">
            <p className="text-xs font-medium text-[#9FB0A6] mb-3">Per Endpoint</p>
              ...
              {Object.entries(health.byEndpoint).map(([endpoint, stats]: [string, any]) => {
                ...
                return (
                  <div key={endpoint} className="flex items-center gap-3">
                    <span className="text-xs text-[#9FB0A6] w-36 truncate">{endpoint}</span>
                    <div className="flex-1 h-2 rounded-full bg-[#101713] overflow-hidden">
                      <div
      ...
      {ingestStats?.bySource && Object.keys(ingestStats.bySource).length > 0 && (
        <div className="p-6 rounded-xl app-card">
          <h2 className="text-sm font-semibold text-[#F0F7F3] mb-4">Ingest Performance (30 days)</h2>
          <div className="space-y-3">
            {Object.entries(ingestStats.bySource).map(([source, stats]: [string, any]) => {
              ...
              return (
                <div key={source} className="flex items-center gap-4 p-3 rounded-lg bg-[#101713]">
                  <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-[#1B4332]/20 to-[#2D6A4F]/20 flex items-center justify-center shrink-0">
                    <Globe className="w-4 h-4 text-[#2D6A4F]" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-[#F0F7F3]">{source}</p>
                    <p className="text-xs text-[#9FB0A6]">{stats.total} leads · {bookRate}% booked</p>
                  </div>
                  <div className="flex items-center gap-3 text-xs">
                    <span className="text-green-400">{stats.booked} booked</span>
                    <span className="text-[#9FB0A6]">·</span>
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          <div key={card.label} className="p-4 rounded-xl app-card">
            {loading ? (
              ...
              <>
                ...
                <div className="text-xl font-bold text-[#F0F7F3]">{card.value}</div>
      ...
      {loading ? (
        <div className="space-y-3">
          {[1, 2, 3, 4].map(i => <div key={i} className="h-16 rounded-xl app-card animate-pulse" />)}
        </div>
      ) : filtered.length === 0 ? (
        <div className="text-center py-16 rounded-xl app-card">
          <Globe className="w-12 h-12 mx-auto mb-4 text-[#9FB0A6]" />
          <h3 className="text-lg font-medium text-[#F0F7F3] mb-2">No webhook sources found</h3>
          <p className="text-sm text-[#9FB0A6]">{search ? "Try a different search" : "No webhook sources configured yet"}</p>
        </div>
      ) : (
        <div className="space-y-2">
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="text-sm font-medium text-[#F0F7F3]">{source.name}</span>
                  <span className={cn(
                    "text-xs px-1.5 py-0.5 rounded",
                    source.active ? "bg-green-500/10 text-green-400" : "bg-gray-500/10 text-[#9FB0A6]"
                  )}>
                    {source.active ? "Active" : "Inactive"}
                  </span>
                  <span className="text-xs text-[#9FB0A6]">{source.type}</span>
                </div>
              ...
              {source.hasParserConfig && (
                <span className="text-xs text-[#2D6A4F]">Custom parser</span>
              )}
            </motion.div>
          ))}
        </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-xs text-[#9FB0A6]">{total} total events</span>
      </div>

      {/* Events */}
      {loading ? (
        <div className="space-y-2">
          {[1, 2, 3, 4, 5].map(i => <div key={i} className="h-14 rounded-xl app-card animate-pulse" />)}
        </div>
      ) : events.length === 0 ? (
        <div className="text-center py-16 rounded-xl app-card">
          <Webhook className="w-12 h-12 mx-auto mb-4 text-[#9FB0A6]" />
          <h3 className="text-lg font-medium text-[#F0F7F3] mb-2">No events found</h3>
          <p className="text-sm text-[#9FB0A6]">Webhook events will appear here as they are received</p>
        </div>
      ) : (
        <div className="space-y-2">
          {events.map((event) => (
            <div key={event.id}
              className="flex items-center gap-3 p-3 rounded-xl app-card text-sm"
            >
              <span className={cn(
                "w-2 h-2 rounded-full shrink-0",
                event.status === "failure" ? "bg-red-400" : "bg-green-400"
              )} />
              <span className="text-[#9FB0A6] w-40 truncate">{event.action}</span>
              <span className="text-[#9FB0A6] flex-1 truncate">
                {event.clientName ? `${event.clientName} · ` : ""}
                {event.errorMessage || event.resourceType || "—"}
              </span>
              <span className="text-[#9FB0A6] text-xs shrink-0">
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2">
          <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
            className="px-3 py-1.5 text-sm rounded-lg app-card app-card-hover text-[#9FB0A6] disabled:opacity-50 hover:bg-white/[0.06]"
          >
            Previous
          </button>
          <span className="text-sm text-[#9FB0A6]">Page {page} of {totalPages}</span>
